...
dispCoef_L = 1 # longitudinal dispersivity (m) # THIS IS SILLY NOTATION.  Should be alpha
dispCoef_T = 1 # transverse dispersivity (m)
ts= np.linspace(0, numTimeStep*dt, numTimeStep)
...
x = xini*np.ones(1, numPoints)
# y = yini.*ones(1,numPoints)
y = yini*np.ones(1, numPoints)
# points0 = [x' y'] #Initial distribution of particles
points0 = [np.transpose(x), np.transpose(y)]
###################################################################################
#########
...
points = np.zeros(len(x), 2, numTimeStep + 1)
